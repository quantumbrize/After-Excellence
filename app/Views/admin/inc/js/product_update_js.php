<script>
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.status) {
                        ...
                        // $('#choices-category-input').html(`<option selected disabled value="${product.category_id}">${product.category}</option>`)
                        html = ``
                        // $.each(product.product_img, function (index, p_img) {
                            html += `<div class="image-container p-2">
                                        <img src="<?= base_url('public/uploads/product_images/')?>${product.product_img[0].src}" style="max-height: 300px; max-width: 400px;">
                                    </div>`
                        // })
                        $('#images').html(html)
                        ...
                        editor2.setData(product.about_product)
                        $('#video-url-input').val(product.video_url)
                        ...
                        $('#product-code-input').val(product.product_code)
                        $('#product-duration-input').val(product.duration)
                        $('#product-starting-date-input').val(product.starting_date)
                        $('#product-max-student-input').val(product.max_student)
                        // $('#contact-input').val(product.contact)
                        // $('#professor-name-input').append(`<option selected value="${product.lecturer_id}">${product.lecturer_name}</option>`)
                        // get_teachers()
        ...
        //                     $('#professor-name-input').append(html)
        //                 })
        //             }
        ...
        $('#product_update_btn').on('click', function () {
            ...
            formData.append('max_student', $('#product-max-student-input').val());
    ...
    function get_categories_By_id(c_id) {
            $.ajax({
                ...
                beforeSend: function () { },
                success: function (resp) {
                    // const categoryInput = $('#choices-category-input');
                    if (resp.status) {
                        console.log(resp)
                        $('#choices-category-input').html(`<option selected disabled value="${resp.data.uid}">${resp.data.name}</option>`)
                        // const option = `<option value="${escapeHtml(resp.data.uid)}">${escapeHtml(resp.data.name)}</option>`;
                        // categoryInput.html(option);
                    }
